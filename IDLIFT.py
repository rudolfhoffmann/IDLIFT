...
Author: Based on LIFT by M. Nauta (https://github.com/M-Nauta/LIFT). Extended to IDLIFT by Rudolf Hoffmann.
"""
import pandas as pd
import numpy as np
import itertools
...
def createlayer(significance, df_itet, generatedtree, seen_parent, parentlist, tolerance):
    ...
    for parent_node in parentlist:
        ...
        # Drop events that are active in EVERY row — the mirror case of the filter above.
        # A constant event carries no information, so it cannot explain anything, yet it
        # passes every test in this file: it is active whenever the parent fires (so the
        # filter above keeps it), and mergeAND/mergePAND make the gate a subset of the
        # parent by construction, so the dominance pre-filter's 'gate fires but parent
        # does not' half is always 0. What PAMH then scores is the parent alone, and the
        # constant child is free padding. Because getavailablesets never builds one-child
        # sets, a genuine single-child relation gets padded with such a constant and is
        # reported as a two-child PAND — e.g. PAND(smcAC_low, smcAC_high) is bit-identical
        # to smcAC_high alone. On the NASA milling data any of 12 constant pairs passed
        # equally well, which is what makes the padding visibly arbitrary.
        constant_events = []
        for col in df_static.columns:
            if col == parent_node or col in seen_parent:
                continue
            if (df_bool[col] == 1).all():
                constant_events.append(col)

        df_static = df_static.drop(columns=dead_events + constant_events)
        if VERBOSE and dead_events:
            print(f"  Excluded (never active when {parent_node}=1): {dead_events}")
        if VERBOSE and constant_events:
            print(f"  Excluded (active in every row, no information): {constant_events}")
